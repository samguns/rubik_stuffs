...
extern "C" {
	...
	typedef BOOLEAN (__stdcall *HidD_SetOutputReport_)(HANDLE handle, PVOID ReportBuffer, ULONG ReportBufferLength);
...
int HID_API_EXPORT HID_API_CALL hid_write_report(hid_device *dev, unsigned char *data, size_t length)
{
	DWORD bytes_written;
	BOOL res;
	...
	OVERLAPPED ol;
	memset(&ol, 0, sizeof(ol));
	res = HidD_SetOutputReport(dev->device_handle, (PVOID)data, length);
